...
const AddCar = () => {
  ...
  const onSubmitHandler = async (e)=>{
    ...
    if (!image || !rcFile || !insuranceFile || !pucFile) {
      toast.error('Please upload all required car documents and the main car image')
      return
    }
      ...
      <form onSubmit={onSubmitHandler} className='flex flex-col gap-5 text-gray-500 text-sm mt-6 max-w-xl'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          <div className='flex flex-col w-full'>
            <label>RC Document <span className='text-red-500'>*</span></label>
            <input type='file' accept='image/*,.pdf' required className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none' onChange={e=> setRcFile(e.target.files[0])} />
          </div>
          <div className='flex flex-col w-full'>
            <label>Insurance Document <span className='text-red-500'>*</span></label>
            <input type='file' accept='image/*,.pdf' required className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none' onChange={e=> setInsuranceFile(e.target.files[0])} />
          </div>
        </div>

        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          <div className='flex flex-col w-full'>
            <label>PUC Document <span className='text-red-500'>*</span></label>
            <input type='file' accept='image/*,.pdf' required className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none' onChange={e=> setPucFile(e.target.files[0])} />
          </div>
          <div className='flex flex-col w-full'>
            <label>Additional Car Images</label>
            <input type='file' accept='image/*' multiple className='px-3 py-2 mt-1 border border-borderColor rounded-md outline-none' onChange={e=> setImages([...e.target.files])} />
          </div>
        </div>
        ...
        <button className='flex items-center gap-2 px-4 py-2.5 mt-4 bg-[#7FFF00] hover:bg-[#6fdc00] text-white rounded-md font-medium w-max cursor-pointer'>
